...
for (x in 1:4) {
  if (x %% 2 == 0) {
    print(max(unlist(history$metrics[x])))
  } else {
    print(min(unlist(history$metrics[x])))
  }
}
```
...
for (x in 1:4) {
  if (x %% 2 == 0) {
    print(max(unlist(history_small$metrics[x])))
  } else {
    print(min(unlist(history_small$metrics[x])))
  }
}
```
...
predictions <- predict(model_small, test_features)
